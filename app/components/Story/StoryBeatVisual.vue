...
<template>
  <div class="relative min-h-[22rem] overflow-hidden rounded-[2.5rem] border border-ink/10 bg-white/45 p-6 shadow-editorial backdrop-blur md:min-h-[32rem] md:p-8">
    ...
    <div v-else-if="visual?.includes('pipeline') || visual === 'branch-release' || visual === 'quirks-box'" class="flex h-full flex-col justify-center gap-6">
      <div class="flex flex-wrap items-center gap-3">
        <template v-for="(stage, stageIndex) in pipelineStages" :key="stage">
          <div class="rounded-full bg-violet-500 px-5 py-4 font-mono text-xs uppercase tracking-[0.18em] text-paper shadow-violet">{{ stage }}</div>
          <div v-if="stageIndex < pipelineStages.length - 1" class="h-px w-12 bg-violet-500" />
        </template>
      </div>
      <div class="grid gap-3 md:grid-cols-2">
        <div class="rounded-[1.5rem] border border-ink/10 bg-paper p-5"><span class="font-mono text-xs uppercase tracking-[0.2em] text-violet-700">develop</span><p class="mt-2 text-3xl font-black tracking-[-0.06em]">prerelease</p></div>
        <div class="rounded-[1.5rem] border border-ink/10 bg-paper p-5"><span class="font-mono text-xs uppercase tracking-[0.2em] text-violet-700">master</span><p class="mt-2 text-3xl font-black tracking-[-0.06em]">stable</p></div>
      </div>
    </div>
    ...
    <div v-else-if="visual === 'workflow-timeline'" class="flex h-full flex-col justify-center gap-4">
      <div v-for="(stage, stageIndex) in mindWorkflowStages" :key="stage" class="grid grid-cols-[3rem_1fr] items-center gap-4">
        ...
        <span class="rounded-full border border-ink/10 bg-paper px-5 py-4 font-mono text-xs uppercase tracking-[0.18em] text-ink">{{ stage }}</span>
      </div>
    </div>

    <div v-else-if="visual === 'model-diagram'" class="grid h-full place-items-center">
      <div class="grid w-full max-w-xl grid-cols-3 items-center gap-3 text-center">
        <div class="rounded-[2rem] bg-ink p-6 text-paper"><p class="font-mono text-xs uppercase tracking-[0.2em]">Opus</p><p class="mt-2 text-3xl font-black">plan</p></div>
        <div class="rounded-full bg-violet-500 px-4 py-3 font-mono text-xs uppercase tracking-[0.2em] text-paper">orchestrates</div>
        <div class="rounded-[2rem] bg-violet-50 p-6 text-violet-700"><p class="font-mono text-xs uppercase tracking-[0.2em]">Sonnet</p><p class="mt-2 text-3xl font-black">work</p></div>
      </div>
    </div>

    <div v-else-if="visual === 'tradeoff-cards'" class="grid h-full content-center gap-4 md:grid-cols-3">
      <div v-for="card in ['upstream first', 'deadline: quick then clean', 'who makes more sense wins']" :key="card" class="rounded-[2rem] bg-violet-50 p-6"><p class="text-3xl font-black tracking-[-0.06em] text-violet-700">{{ card }}</p></div>
    </div>

    <div v-else-if="visual === 'red-flags'" class="grid h-full content-center gap-3">
      <p v-for="flag in ['blind vibe coding', 'new packages without research', 'god components', 'utility dumps', 'duplicated logic', 'abbreviations']" :key="flag" class="text-4xl font-black tracking-[-0.06em] text-ink line-through decoration-violet-500 decoration-[0.16em]">{{ flag }}</p>
    </div>

    <div v-else-if="visual === 'aida-loop' || visual === 'v1-track' || visual === 'status-strip'" class="flex h-full flex-col justify-center gap-8">
      <div class="flex flex-wrap gap-3">
        <span v-for="step in ['Ingest', 'Transform', 'Plot', 'Refine', 'Export']" :key="step" class="rounded-full bg-ink px-6 py-4 font-mono text-xs uppercase tracking-[0.2em] text-paper">{{ step }}</span>
      </div>
      <div class="grid gap-3 md:grid-cols-5">
        <div v-for="status in ['01 delivery', '02 ticketed', '03 ticketed', '04 next', '05 planned']" :key="status" class="rounded-[1.4rem] border border-violet-500/20 bg-violet-50 p-4 font-mono text-xs uppercase tracking-[0.16em] text-violet-700">{{ status }}</div>
      </div>
    </div>
    ...
    <div v-else-if="visual === 'cvc-monolith' || visual === 'cvc-split' || visual === 'migration-steps' || visual === 'idea-stamp'" class="grid h-full content-center gap-6">
      ...
      <div class="grid gap-3 md:grid-cols-5">
        <div v-for="step in migrationSteps" :key="step" class="rounded-[1.2rem] bg-violet-50 p-4 text-center font-mono text-xs uppercase tracking-[0.14em] text-violet-700">{{ step }}</div>
      </div>
      <div class="mx-auto rotate-[-4deg] rounded-xl border-4 border-violet-500 px-6 py-3 font-mono text-xl font-black uppercase tracking-[0.2em] text-violet-700">Idea — pitch to Kevin</div>
    </div>
    ...
    <div v-else-if="visual?.includes('ledger')" class="grid h-full content-center gap-3">
      <div v-for="(item, itemIndex) in ledgerItems" :key="item" class="grid grid-cols-[3rem_1fr] items-center gap-3 rounded-[1.4rem] border border-ink/10 bg-paper p-4">
        ...
        <span class="font-mono text-xs uppercase tracking-[0.18em] text-ink">{{ item }}</span>
      </div>
    </div>

    <div v-else-if="visual === 'handover-split' || visual === 'whatsapp-line'" class="grid h-full place-items-center">
      <div class="grid w-full max-w-2xl grid-cols-[1fr_auto_1fr] items-center gap-5 text-center">
